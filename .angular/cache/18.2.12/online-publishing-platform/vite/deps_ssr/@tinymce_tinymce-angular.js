...
var listenTinyMCEEvent = (editor, eventName, destroy$) => (0, import_rxjs.fromEvent)(editor, eventName).pipe((0, import_rxjs.takeUntil)(destroy$));
...
var firstEmission = () => (source) => source.pipe((0, import_rxjs.first)(), (0, import_rxjs.map)(() => void 0));
var CreateScriptLoader = () => {
  const params$ = new import_rxjs.BehaviorSubject(null);
  const loaded$ = params$.pipe(
    (0, import_rxjs.filter)(Boolean),
    (0, import_rxjs.switchMap)(([doc, url]) => {
      const scriptTag = doc.createElement("script");
      scriptTag.referrerPolicy = "origin";
      scriptTag.type = "application/javascript";
      scriptTag.src = url;
      doc.head.appendChild(scriptTag);
      return (0, import_rxjs.fromEvent)(scriptTag, "load").pipe(firstEmission());
    }),
  ...
  ngAfterViewInit() {
    if (isPlatformBrowser(this.platformId)) {
      ...
      if (getTinymce() !== null) {
        ...
        ScriptLoader.load(this._element.ownerDocument, this.getScriptSrc()).pipe((0, import_rxjs.takeUntil)(this.destroy$)).subscribe(this.initialise);
      }
    }
  }
  ngOnDestroy() {
